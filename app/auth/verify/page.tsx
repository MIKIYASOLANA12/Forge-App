...
function VerifyContent() {
  ...
  useEffect(() => {
    if (!token) {
      setStatus("error");
      setErrorMessage("No activation token provided in the link.");
      return;
    }
    ...
    const verifyToken = async () => {
      try {
        ...
        if (res.ok && data.success) {
          ...
          setStatus("error");
          setErrorMessage(data.error || "Activation failed or token has expired.");
        }
      } catch (err: any) {
        setStatus("error");
        ...
        {status === "verifying" && (
          <div className="py-6 space-y-4">
            ...
            <h2 className="text-base font-semibold text-[var(--text-primary)]">
              Verifying your Forge token...
            </h2>
            <p className="text-xs text-[var(--text-muted)]">
              Validating cryptographic single-use signature.
            </p>
          </div>
        )}
        ...
        {status === "success" && (
          <div className="py-6 space-y-4 animate-fade-in">
            ...
            <p className="text-sm text-[var(--text-secondary)]">
              Your Forge session is now authenticated. Redirecting to your dashboard...
            </p>
            <div className="pt-2">
...
export default function VerifyPage() {
  return (
    <div className="min-h-screen flex items-center justify-center bg-[var(--bg-base)] px-4 py-12 relative">
      <Suspense fallback={<div className="text-sm text-[var(--text-muted)]">Verifying activation...</div>}>
        <VerifyContent />
      </Suspense>
    </div>
  );
