...
type Targets = { targetCalories: number; targetProtein: number; targetCarbs: number; targetFat: number };
...
const initialTargets: Targets = { targetCalories: 2500, targetProtein: 150, targetCarbs: 300, targetFat: 80 };
const nutrients = [
  ["Calories", "targetCalories", "calories", "kcal"],
  ["Protein", "targetProtein", "protein", "g"],
  ["Carbs", "targetCarbs", "carbs", "g"],
  ["Fat", "targetFat", "fat", "g"],
] as const;
...
export default function MealsPage() {
  ...
  return <div className="mx-auto w-full max-w-[1400px] animate-fade-in pb-10">
    ...
    {(targets.targetProtein - totals.protein > 40 || targets.targetCalories - totals.calories > 500) && <section className="mb-5 border-l-2 border-[var(--xp-gold)] bg-[rgba(245,158,11,0.06)] px-4 py-3 text-sm">{targets.targetProtein - totals.protein > 40 && <p>You&apos;re {Math.round(targets.targetProtein - totals.protein)}g short on protein. High-protein options: eggs, chicken, lentils, fish.</p>}{targets.targetCalories - totals.calories > 500 && <p className="mt-1">You&apos;re {Math.round(targets.targetCalories - totals.calories)} calories short. Consider adding a balanced meal.</p>}</section>}
