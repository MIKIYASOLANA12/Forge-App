...
export async function POST(req: NextRequest) {
  try {
    ...
    const { action, subject, topicId, count, sessionId, questionId, userAnswer, timeTakenSec } = body;
    ...
    if (action === 'START' || action === 'CREATE') {
      ...
      // Check if there's already an active session to resume seamlessly
    ...
    // 2. SUBMIT SINGLE ANSWER (Server-side validation)
    ...
    // 3. GET SESSION STATE (for browser reload / persistence recovery)
